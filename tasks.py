...
def clean_screenshots_dir(dir_path="./output/screenshots"):
    if os.path.isdir(dir_path):
        shutil.rmtree(dir_path)


def clean_receipts_dir(dir_path="./output/receipts"):
    if os.path.isdir(dir_path):
        shutil.rmtree(dir_path)
...
def download_orders_csv_file():
    http = HTTP()
    http.download(url="https://robotsparebinindustries.com/orders.csv", overwrite=True, target_file="./output/data/orders.csv")


def open_robot_order_website():
    browser.goto("https://robotsparebinindustries.com/#/robot-order")
    log.info("Robot Spare Bin Industries Inc. website opened.")


def fill_and_submit_order_form(robot_order: RobotOrder):
    page = browser.page()
    page.wait_for_selector("button:text('OK')", timeout=10000)
    page.click("button:text('OK')")

    log.info("Filling the robot order form...")
    page.wait_for_selector("#head", timeout=10000)
    selects = page.query_selector_all("#head")
    dropdown = selects[0]
    dropdown.select_option(robot_order.head)
    ...
    if page.locator("div[class='alert alert-danger'][role='alert']").is_visible():
        log.warn(f"Order submission failed for robot {robot_order}. Retrying...")
        page.wait_for_timeout(1000)
        page.reload()
...
def save_robot_screenshot(robot_order: RobotOrder):
    page = browser.page()
    screenshot_path = f"./output/screenshots/robot_{robot_order.order_number}.png"
    ...
    log.info("Robot screenshot saved at: %s", screenshot_path)


def write_receipt_to_pdf(order_id: str, order_timestamp: str, order_html: str, screenshot_path: str, order_address: str):
    pdf_path = f"./output/receipts/receipt_{order_id}.pdf"
    log.info("Saving receipt to PDF: %s", pdf_path)
...
def generate_robot_order_receipt(screenshot_path: str):
    page = browser.page()
    ...
    write_receipt_to_pdf(order_id, order_timestamp, parts_html, screenshot_path, order_address)


def fill_form_with_csv_data():
    table = Tables()
    rows = table.read_table_from_csv("./output/data/orders.csv", delimiters=",", header=True)
    log.info("CSV data loaded from orders.csv: %s", rows)
    ...
    for row in rows:
        ...
        log.info("Filling the order form with data: %s", robot_order)
        fill_and_submit_order_form(robot_order)
        save_robot_screenshot(robot_order)
        generate_robot_order_receipt(screenshot_path=f"./output/screenshots/robot_{robot_order.order_number}.png")
        order_another_robot()
